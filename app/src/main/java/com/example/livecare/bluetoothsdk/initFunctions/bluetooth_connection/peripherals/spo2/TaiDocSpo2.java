...
public class TaiDocSpo2 {
    private final BluetoothConnection bluetoothConnection;
    private BleDevice bleDevice;
    private String turnOffDevice = "515000000000a3";

    public TaiDocSpo2(BluetoothConnection bluetoothConnection, BleDevice device, BluetoothGatt gatt) {
                    ...
                    public void onCharacteristicChanged(byte[] data) {
                        if (HexUtil.formatHexString(data).length() > 4) {
                            ...
                            if (HexUtil.formatHexString(data).substring(0, 4).equalsIgnoreCase("5133")) {//date and time was set
                                startWriteCommand(characteristic, checkSum("514900000000a3"));
                            }
    ...
    private void calculateGlResult(String value, BluetoothGattCharacteristic characteristic) {
        if (value.startsWith("5149")) {
            ...
            if (resultOxygen.equals("0") || resultPulseRate.equals("0")) {
                new Handler(Looper.getMainLooper()).postDelayed(() -> startWriteCommand(characteristic, checkSum("514900000000a3")), 5000);
            } else {
